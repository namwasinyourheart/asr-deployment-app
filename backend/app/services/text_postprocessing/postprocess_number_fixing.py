...
def normalize_number_sequence(span_text: str) -> str:
    """
    Normalize Vietnamese number-word sequences to digits according to heuristics that
    match the examples provided.

    Rules implemented (priority order):
    1. Fix common typos (mười/mươi, linh/lẻ).
    2. If the whole span is purely digit-words (or digits) and length >= 7 -> treat as phone-like and join digits (preserve leading zeros).
    3. If a connector (và|với|,) exists -> split on connector and normalize each part independently (then join with spaces).
    4. If token count <= 2 AND none of tokens are scale-words (mười/mươi/trăm/nghìn/triệu/tỷ) -> do NOT normalize (keeps cases like "năm bảy").
    5. Try parsing the entire span with vietnamese_to_number():
       - If it parses to a number:
         * If any **same** big-scale word repeats (e.g., 'trăm' appears 2+ times) -> treat as likely multiple adjacent numbers -> DO NOT normalize (keep original).
         * Otherwise -> return the parsed number as digits (e.g., "hai trăm linh năm" -> "205", "mười ba mươi lăm" -> "28", "năm mươi triệu hai trăm ngàn" -> "50200000").
    6. If whole-parse fails -> try DP segmentation (prefer longer windows). If segmentation found:
       - If segmentation has >=2 segments and all segment values < 100 and total token count >=3 -> return sum of segments (e.g., "mười ba mười lăm" -> 28).
       - Otherwise -> ambiguous -> return original text.
    """
    s_orig = span_text
    s = span_text.lower().strip()

    # 1) typo fixes (token-level)
    typo_map = {
        "ninh": "linh", "nẻ": "lẻ",
        "mưoi": "mười", "mưoii": "mười", "mừoi": "mười",
        "mưoi": "mười",  # defensive duplicates OK
    }
    ...
    # helper: is digit-word
    def is_digit_word(tok):
        return re.fullmatch(r"\d+", tok) or tok in PHONE_DIGIT_WORDS

    # 2) phone-like: all tokens digit-words and sufficiently long
    if all(is_digit_word(t) for t in tokens):
        if len(tokens) >= 7:
            digits = []
            for t in tokens:
                if re.fullmatch(r"\d+", t):
                    ...
                    digits.append(str(VI_NUM_MAP.get(t, "")))
            return "".join(digits)
        else:
            return s_orig  # too short, leave as-is

    # 3) connector split
    connector_re = re.compile(r"\b(?:và|với|,)\b", flags=re.I)
    if connector_re.search(s):
        parts = [p.strip() for p in connector_re.split(s) if p.strip()]
        normalized_parts = []
        for p in parts:
            # avoid infinite recursion by using a simplified path:
            p_norm = normalize_number_sequence(p) if p != s else s
            normalized_parts.append(p_norm)
        return " ".join(normalized_parts)

    # 4) avoid normalizing very short plain-unit sequences like "năm bảy"
    small_scale_words = {"mười", "mươi"}
    big_scale_words = {"trăm", "nghìn", "ngàn", "triệu", "tỷ"}
    if len(tokens) <= 2:
        if not any(t in small_scale_words or t in big_scale_words for t in tokens):
            # e.g., "năm bảy" -> keep original
            return s_orig

    # 5) Try whole parse
    whole_num = None
    try:
        whole_num = vietnamese_to_number(s)
    except Exception:
        whole_num = None

    if whole_num is not None:
        # count occurrences of each big scale word
        counts = {}
        for w in big_scale_words:
            counts[w] = sum(1 for t in tokens if t == w)
        # if any same big-scale repeats (e.g., 'trăm' twice) and there is no explicit connector,
        # treat as likely multiple adjacent full numbers -> do NOT normalize
        if any(cnt >= 2 for cnt in counts.values()):
            return s_orig
        # otherwise accept whole parse
        return str(whole_num)

    # 6) Whole parse failed -> do DP segmentation (prefer longer windows first)
    n = len(tokens)
    MAX_WINDOW = min(6, n)
    dp = [None] * (n + 1)
    dp[0] = []
    ...
    for i in range(n):
        if dp[i] is None:
            continue
        # prefer longer windows first to avoid splitting "mười ba" into "mười" + "ba"
        for l in range(MAX_WINDOW, 0, -1):
            if i + l > n:
                continue
            cand = " ".join(tokens[i:i + l])
            nv = vietnamese_to_number(cand)
            if nv is not None:
                new_seg = dp[i] + [(cand, nv, l)]
                if dp[i + l] is None:
                    dp[i + l] = new_seg
                else:
                    # prefer fewer segments, then smaller max segment length
                    if len(new_seg) < len(dp[i + l]):
                        dp[i + l] = new_seg
                    elif len(new_seg) == len(dp[i + l]):
                        if max(seg[2] for seg in new_seg) < max(seg[2] for seg in dp[i + l]):
                            dp[i + l] = new_seg
    ...
    # If single segment found, return it
    if len(seg) == 1:
        return str(seg[0][1])

    texts = [g[0] for g in seg]
    values = [g[1] for g in seg]
    total_tokens = sum(g[2] for g in seg)

    # If segmentation gives multiple segments and ALL segments have values < 100 and total_tokens >= 3
    # combine by SUM (covers "mười ba mười lăm" -> 13 + 15 = 28)
    if all(v < 100 for v in values) and total_tokens >= 3:
        return str(sum(values))

    # If segmentation yields multiple segments but each segment contains a big-scale word,
    # it's ambiguous whether user meant multiple numbers — prefer to keep original (matches examples).
    if all(any(w in t.split() for w in big_scale_words) for t in texts):
        return s_orig
